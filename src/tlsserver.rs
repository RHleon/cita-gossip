...
//handle which message has been taken
fn handle_message(mut event:event) {
    let mut buffer = [0; 1024];
    // buffer 拼接成报文的写法
    event.read(&mut buffer).unwrap();

    let list = b"gossip list";
    let data = b"gossip data";
    let isalive = b"gossip live";
    let emptysend=b"gossip emptysend";

    if buffer.starts_with(list) {
        msg::receive(buffer - list) // 调用msg中的receive
    }
    if buffer.starts_with(data) {
        msg::receive(buffer - data) // 调用msg中的receive
    }
    if buffer.starts_with(isalive) {
        msg::receive(buffer - isalive) // 调用msg中的receive
    }
    if buffer.starts_with(emptysend) {
        msg::receive(buffer - emptysend) // 调用msg中的receive
    }
    //其他类似
}
...
fn main() {
    ...
    let mut events = mio::Events::with_capacity(1024);  // create storage for event
    ...
    loop {
        ...
        for event in events.iter() {
            match event.token() {
                ...
                _ => handle_message(&event)
            }
        }
    }
}
>>>>>>> shelgor:src/server.rs
